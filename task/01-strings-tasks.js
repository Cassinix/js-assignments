...
function getRectangleString(width, height) {
    const s = "│"+" ".repeat(width-2)+"│\n";
    return "┌"+"─".repeat(width-2)+"┐\n"+s.repeat(height-2)+"└"+"─".repeat(width-2)+"┘\n";
}
...
function encodeToRot13(str) {
    const alphbet = 'AaBbCcDdEeFfGgHhIiJjKkLlMmNnOoPpQqRrSsTtUuVvWwXxYyZzAaBbCcDdEeFfGgHhIiJjKkLlMmNnOoPpQqRrSsTtUuVvWwXxYyZz'.split('');
    const arr = str.split('');
    return arr.map(elem => {
        const inx = alphbet.indexOf(elem);
       return inx != -1 ? alphbet[inx + 26]: elem;
    }).join('');
}
